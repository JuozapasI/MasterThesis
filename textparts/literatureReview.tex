...
In the next sections, I will shortly introduce genome biology and further expand on transcriptomic references.

\section{Genome biology}

The 

Genes are generally defined as genomic regions transcribed into functional RNAs.
For some RNAs, parts of the transcript are removed in a process known as splicing.
The discarded segments are called introns, while the retained, coding parts are called exons.
...
Transcription is carried out by RNA polymerases—enzymes that synthesize RNA molecules from a DNA template.
This process can be divided into three stages:
\begin{itemize}
  \item \textbf{Initiation} – RNA polymerase recognizes the promoter region at the start of a gene and binds to it.
  The enzyme then unwinds the DNA duplex, begins RNA synthesis, and escapes the promoter (\cite{Cramer2019}).
  \item \textbf{Elongation} – RNA polymerase moves along the DNA template, synthesizing the RNA molecule.
  \item \textbf{Termination} – Upon encountering a termination signal,
  the polymerase detaches from the DNA and releases the completed RNA molecule, marking the end of transcription (\cite{Lodish2021}).
\end{itemize}
Each of these steps involves additional proteins and RNA molecules, collectively known as transcription factors.
For example, RNA polymerases cannot independently recognize promoter sequences,
instead, specific transcription factors are required to recruit the polymerase to the promoter (\cite{Cramer2019}).

\paragraph{Chromatin}

Chromatin is a dynamic complex of DNA and histone proteins that packages the eukaryotic genome within the nucleus.
Its fundamental unit, the nucleosome, comprises approximately 147 base pairs of DNA wrapped around a histone octamer,
enabling efficient genome compaction and regulation (\cite{Alberts2022}).
Chromatin exists in a spectrum of conformational states, from tightly packed heterochromatin (generally transcriptionally inactive)
to more relaxed euchromatin (transcriptionally active).
Accessible chromatin refers to regions where DNA is less tightly associated with histones,
permitting interactions with transcription factors and other regulatory proteins.
Although accessible chromatin constitutes only 2–3\% of the total genome sequence,
it accounts for over 90\% of known transcription factor binding sites (\cite{Klemm2019}).
Techniques such as ATAC-seq and DNase-seq are widely used to map these regions,
offering insights into gene regulation and cellular identity (\cite{Minnoye2021}).
