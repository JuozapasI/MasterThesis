...
\begin{figure}
  ...
  C) The mature long non-coding RNA consists of only non-coding exons.
...
As mentioned above, transcription factors play important role in the transcri[tion regulation.
...
\paragraph{Gene types}

Transcribed RNA molecules have very different functions and fates, hence a classification is needed.
The most broad classification defines genes as either protein-coding or non-coding.
The protein coding genes is the most studied group of genes, most have common features,
such as 3' and 5' UTRs, open reading frames (ORFs).
They encode 
...
It is worth noting that only a small portion of the human genome is transcribed.
For example, in the GENCODE v47 annotation, exons span only about 6.5\% of the genome sequence.

Beyond clearly functional genomic elements (e.g., protein-coding genes),
there are non-coding regions that are still functional, such as transcription promoter regions.
Additionally, there are regions that are transcribed but whose function remains unknown or may be nonfunctional (e.g., transcribed pseudogenes).

Not all regions of the eukaryotic genome — especially the human genome — are unique.
In humans, more than half of the genome consists of repetitive sequences (\cite{Liehr2021}).
These were historically dismissed as 'junk', 'selfish', or 'parasitic' DNA.
However, later studies have shown that repetitive sequences can serve structural roles
or contribute to genomic stability through redundancy (\cite{Shapiro2005}).
Many of these repeats originate from transposons — mobile genetic elements capable of copying or repositioning themselves within the genome.
Although transposons were initially considered parasitic,
growing evidence suggests they play a generative role in genome evolution and should not be dismissed as 'junk' DNA (\cite{Fedoroff2012}).
\fi 



\paragraph{Non-coding RNAs}
...
Housekeeping ncRNAs include rRNAs, tRNAs, and small nuclear RNAs, while regulatory ncRNAs—particularly lncRNAs —
...
lncRNAs are often transcribed from intergenic regions, but they can also arise from introns,
antisense strands, pseudogenes, and enhancers (\cite{Poliseno2024}).
