...
The next steps in the analysis of the scRNA-seq data involve following steps (see Figure \ref{fig:dataAnalysis} for graphical summary):
...
which do not provide useful information that could be useful in, for instance, grouping cells or determining cell types.
...
to reduce the dimensions of the data loosing as little information as possible.
...
All downstream analyses rely heavily on the initial processing steps, particularly read mapping, which depends on the transcriptomic reference. 
...
An organism’s biological features are largely encoded in its genome.
The genome is typically defined as the complete set of genetic material (usually DNA) of an organism.
DNA is a double-stranded helical molecule composed of four nucleotides: adenine (A), cytosine (C), guanine (G), and thymine (T).
The strands are complementary: adenine pairs with thymine, and cytosine pairs with guanine (\cite{Alberts2022}).
DNA serves as a template for RNA transcription, which can then be translated into proteins or function directly as RNA (e.g., regulatory RNAs).
