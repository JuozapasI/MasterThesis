...
def find_ATrich_segment_upstream(sequence, strand, min_length=10):
    if strand == "+":
        seq = sequence[:1050]
        for i in range(len(seq) - min_length, -1, -1):
            window = seq[i:i + min_length]
...
with open(sequence_file, "r") as sequences, open(output_file, "w") as output:
    for line in sequences:
        ...
        AT_distance_upstream, segment2_len = find_ATrich_segment_upstream(sequence, strand)

        output.write(f"{region}\t{tata_distance}\t{AT_distance_downstream}\t{segment_len}\t{AT_distance_upstream}\t{segment2_len}\n")
