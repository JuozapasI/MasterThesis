\documentclass[11pt]{report}
\usepackage[left=2cm, right=2cm, top=2.54cm, bottom=2.54cm, a4paper]{geometry}
\usepackage{titling}
\usepackage{setspace}
\usepackage{titlesec}
\usepackage{graphicx}
\usepackage[breaklinks]{hyperref}
\usepackage{subcaption}
\usepackage{booktabs}
\usepackage{longtable}
\usepackage[T1]{fontenc}

\usepackage[style=authoryear, backend=biber, defernumbers=true]{biblatex}
\addbibresource{references.bib}

\hypersetup{
    colorlinks=true,
    linkcolor=blue,
    citecolor=blue,
    filecolor=blue,
    urlcolor=blue
}

\onehalfspacing

\usepackage{fancyhdr}
\fancypagestyle{plain}{
  \fancyhf{} % Clear all header and footer fields
  \fancyfoot[C]{\thepage} % Page number in the lower right corner
  \renewcommand{\headrulewidth}{0pt} % No header rule
  \renewcommand{\footrulewidth}{0pt} % No footer rule
}

\titleformat{\chapter}[block]{\normalfont\fontsize{16pt}{24pt}\selectfont\bfseries\centering}{\thechapter.}{20pt}{}

% Numbering of references in bibliography
\DeclareFieldFormat{labelnumberwidth}{\mkbibbrackets{#1}}
\renewbibmacro{begentry}{(\printfield{labelnumber})\space}

\usepackage{indentfirst}

\begin{document}

\begin{titlepage}
  \centering
  \vspace*{3cm}
  {\fontsize{16pt}{24pt}\selectfont ENHANCING SINGLE-CELL RNA-SEQ ANALYSIS THROUGH THE INTEGRATION OF UNACCOUNTED INTERGENIC REGIONS\par}
  {\fontsize{12pt}{24pt}\selectfont Master Thesis\par}
  {\fontsize{12pt}{24pt}\selectfont Systems Biology Master Program\par}
  {\fontsize{12pt}{24pt}\selectfont Vilnius University\par}
  \vspace*{5cm}
  \begin{tabular}{@{}lll@{}}
    \textbf{STUDENT NAME:} & \hspace{3cm} & Juozapas Ivanauskas \\
    \textbf{STUDENT NUMBER:} & & 2316457 \\
    \\
    \textbf{SUPERVISOR:} & & Simonas Juzėnas, PhD \\
    \textbf{CONSULTANT:} & & Justina Žvirblytė, MSc \\
    \\
    \textbf{SUPERVISOR DECISION:} & & \dotfill \\
    \\
    \\
    \\
    \textbf{FINAL GRADE} & & \dotfill \\
    \\
    \textbf{DATE OF SUBMISSION:} & & 14 May 2025 \\
  \end{tabular}

\end{titlepage}

\tableofcontents

\chapter{LIST OF ABBREVIATIONS}
\input{"textparts/abbreviations.tex"}

\chapter{INTRODUCTION}
\input{"textparts/introduction.tex"}

\chapter{AIM AND TASKS}
\input{"textparts/aimAndTasks.tex"}

\chapter{LITERATURE REVIEW}
\input{"textparts/literatureReview.tex"}

\chapter{METHODS}
\input{"textparts/methods.tex"}

\chapter{RESULTS}
\input{"textparts/results.tex"}

\chapter{DISCUSSION}
\input{"textparts/discussion.tex"}

\chapter{CONCLUSIONS}
\input{"textparts/conclusions.tex"}

%\chapter{RECOMMENDATION}
%\input{"textparts/recommendation.tex"}

\chapter{DISCLAIMER}
\input{"textparts/disclaimer.tex"}

\chapter{ACKNOWLEDGEMENTS}
\input{"textparts/acknowledgements.tex"}

\chapter{REFERENCES}
\printbibliography[heading=none]

\chapter{SUMMARY}
\input{"textparts/summary.tex"}

\chapter{SUMMARY IN LITHUANIAN}
\input{"textparts/summaryLithuanian.tex"}

\chapter{APPENDICES}
\input{"textparts/appendices.tex"}

\end{document}
